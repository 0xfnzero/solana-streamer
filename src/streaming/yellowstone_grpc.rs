...
impl YellowstoneGrpc {
    ...
    /// Enhanced immediate event subscription with raw gRPC transaction data access
    ///
    /// # Parameters
    /// * `protocols` - List of protocols to monitor
    /// * `bot_wallet` - Optional bot wallet address for filtering related transactions
    ...
    /// * `event_filter` - Optional event filter for further event filtering, no filtering if None
    /// * `commitment` - Optional commitment level, defaults to Confirmed
    /// * `enhanced_callback` - Enhanced callback function that receives both parsed events and raw gRPC transaction data
    ///
    /// # Returns
    /// Returns `AnyResult<()>`, `Ok(())` on success, error information on failure
    pub async fn subscribe_events_immediate_with_raw_data<F>(
        &self,
        protocols: Vec<Protocol>,
        bot_wallet: Option<Pubkey>,
        transaction_filter: Vec<TransactionFilter>,
    ...
    where
        F: Fn(Box<dyn UnifiedEvent>, Option<&solana_transaction_status::EncodedTransactionWithStatusMeta>) + Send + Sync + 'static,
    {
        use crate::streaming::event_parser::core::traits::EnhancedEventCallback;
        ...
        *self.control_tx.lock().await = Some(control_tx);
        ...
        // 启动流处理任务
        let mut event_processor = self.event_processor.clone();
        let enhanced_callback: EnhancedEventCallback = Arc::new(enhanced_callback);
        event_processor.set_protocols_and_event_type_filter_with_enhanced_callback(
            super::common::EventSource::Grpc,
            protocols,
            event_type_filter,
            self.config.backpressure.clone(),
            Some(enhanced_callback),
        );
        let stream_handle = tokio::spawn(async move {
            loop {
                tokio::select! {
